...
public abstract class RunTransaction {
    ...
    public RunTransaction(ReplicatedEnvironment repEnv, PrintStream printStream) {
    	this(Command.NONE, repEnv, printStream);
    }
    
    public RunTransaction(Command command, ReplicatedEnvironment repEnv, PrintStream printStream) {
        env = repEnv;
        out = printStream;
        cmd = command;
    }
    ...
    public void run(boolean readOnly)
        ...
        for (int i = 0; i < TRANSACTION_RETRY_MAX; i++) {
            ...
            try {
                ...
                doTransactionWork(txn, cmd); /* CALL APP-SPECIFIC CODE */
                txn.commit();
                success = true;
                return;
     ...
     * @param cmd
     */
    public abstract void doTransactionWork(Transaction txn, Command cmd);
